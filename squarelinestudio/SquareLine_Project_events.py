
def led_effect_dropdown_event_cb(event_struct):
    return


def num_of_leds_event_cb(event_struct):
    return


def colorslider_event_cb(event_struct):
    return


def temp_sensor_dropdown_event_cb(event_struct):
    return


def SaveLedSettings(event_struct):
    return


def fan1_label_dropdown_event(event_struct):
    return


def fan2_label_dropdown_event(event_struct):
    return


def pump1_label_dropdown_event(event_struct):
    return


def pump2_label_dropdown_event(event_struct):
    return


def temp1_label_dropdown_event(event_struct):
    return


def temp2_label_dropdown_event(event_struct):
    return


def colorwheel_event_cb(event_struct):
    return


def led_alert_event_cb(event_struct):
    return


def SaveAlertSettings(event_struct):
    return


def bg_dropdown_change_event(event_struct):
    return


def flip_screen_cb_change_event(event_struct):
    return


def SaveSettings(event_struct):
    return


def theme_dropdown_change_event(event_struct):
    return


def PWM1Change(event_struct):
    return


def PWM1KeepMinimumValue(event_struct):
    return


def PWM2Change(event_struct):
    return


def PWM2KeepMinimumValue(event_struct):
    return


def SaveFanPWMSettings(event_struct):
    return

